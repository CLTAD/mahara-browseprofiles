...
(function (BrowseProfiles, $, undefined) {
    //Private Property
    var loadingmessage;

    //Public Method
    BrowseProfiles.filtercontent = function(browsetype, limit, offset) {
        ...
        var pd = {'filter': 1, 'limit': limit, 'offset': offset};
        $.each(filters, function(name, value) {
            pd[name] = value;
        });

        loadingmessage.removeClass('hidden');        
        sendjsonrequest(config['wwwroot'] + 'artefact/browseprofiles/browseprofiles.json.php', pd, 'POST', function(data) {
            loadingmessage.addClass('hidden');
            $('#gallery').replaceWith(data.data.tablerows);
            $('#browseprofileslist_pagination').html(data.data.pagination);
    ...
    function connect_autocomplete() {
        ...
        $('#filter-course').autocomplete({
            ...
            source: function(request, response) {
                ...
                sendjsonrequest(config['wwwroot'] + 'artefact/browseprofiles/autocomplete.json.php', pd, 'POST', function(data) {
    ...
    function refresh_content(offset) {
        ...
        sendjsonrequest(config['wwwroot'] + 'artefact/browseprofiles/browseprofiles.json.php', pd, 'POST', function(data) {
                ...
                $('#gallery').replaceWith(data.data.tablerows);
                if (!$('#browseprofileslist_pagination').length) {
...
}( window.BrowseProfiles = window.BrowseProfiles || {}, jQuery ));
